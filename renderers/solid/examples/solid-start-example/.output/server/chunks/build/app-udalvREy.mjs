...
const _e = /^(?:[a-z0-9]+:)?\/\//i, je = /^\/+|(\/)\/+$/g, oe = "http://sr";
function _(e, t = false) {
  const r = e.replace(je, "$1");
...
const We = 100, $e = createContext(), ce = createContext();
function Me(e, t = "") {
...
function ze(e, t, r, o = {}) {
  ...
  }, [u, l] = createSignal(n().value), [v, w] = createSignal(n().state), A = Ne(u, v, s.queryWrapper), S = [], E = createSignal(isServer ? we() : []), k = createMemo(() => typeof o.transformUrl == "function" ? j(t(), o.transformUrl(A.pathname)) : j(t(), A.pathname)), z = () => {
    const f = k(), p = {};
    for (let R = 0; R < f.length; R++) Object.assign(p, f[R].params);
    return p;
  }, fe = s.paramsWrapper ? s.paramsWrapper(z, t) : se(z), V = { pattern: d, path: () => d, outlet: () => null, resolvePath(f) {
    return D(d, f);
  } };
  return createRenderEffect(on(n, (f) => b("native", f), { defer: true })), { base: V, location: A, params: fe, isRouting: y, renderPath: h, parsePath: i, navigatorFactory: me, matches: k, beforeLeave: c, preloadRoute: ge, singleFlight: o.singleFlight === void 0 ? true : o.singleFlight, submissions: E };
  function de(f, p, R) {
    untrack(() => {
      if (typeof p == "number") {
      ...
      const C = !p || p[0] === "?", { replace: B, resolve: O, scroll: T, state: U } = { replace: false, resolve: !C, scroll: true, ...R }, F = O ? f.resolvePath(p) : D(C && A.pathname || "", p);
      if (F === void 0) throw new Error(`Path '${p}' is not a routable path`);
      if (S.length >= We) throw new Error("Too many redirects");
      const J = u();
      if (F !== J || U !== v()) if (isServer) {
        const X = getRequestEvent();
        X && (X.response = { status: 302, headers: new Headers({ Location: F }) }), a({ value: F, replace: B, scroll: T, state: U });
      } else c.confirm(F, R) && (S.push({ value: J, replace: B, scroll: T, state: v() }), b("navigate", { value: F, state: U }));
    });
  }
  function me(f) {
  ...
  function ge(f, p) {
    ...
    x = "preload";
    for (let B in R) {
      const { route: O, params: T } = R[B];
      O.component && O.component.preload && O.component.preload();
      const { preload: U } = O;
      p && U && runWithOwner(r(), () => U({ params: T, location: { pathname: f.pathname, search: f.search, hash: f.hash, query: ae(f), state: null, key: "" }, intent: "preload" }));
    }
    x = C;
  }
  function we() {
  ...
  const o = createMemo(on(e.routerState.matches, (n, a, s) => {
    ...
    for (let c = 0, d = n.length; c < d; c++) {
      ...
      s && y && m.route.key === y.route.key ? h[c] = s[c] : (i = false, t[c] && t[c](), createRoot((g) => {
        t[c] = g, h[c] = Ve(e.routerState, h[c - 1] || e.routerState.base, G(() => o()[c + 1]), () => {
          var _a;
...
function at(e) {
  ...
  return Ye({ get: t, set({ value: o, replace: n, scroll: a, state: s }) {
    n ? window.history.replaceState(ke(s), "", o) : window.history.pushState(s, "", o), et(decodeURIComponent(window.location.hash.slice(1)), a), H();
  }, init: (o) => Ze(window, "popstate", qe(o, (n) => {
...
var st = ["<nav", '><a href="/">Home</a><a href="/a2ui">A2UI Demo</a></nav>'];
function vt() {
  return createComponent(at, { root: (e) => createComponent(I, { get children() {
    return [createComponent(k, { children: "A2UI SolidStart Example" }), ssr(st, ssrHydrationKey()), createComponent(Suspense, { get children() {
      return e.children;
    } })];
  } }), get children() {
...
//# sourceMappingURL=app-udalvREy.mjs.map
